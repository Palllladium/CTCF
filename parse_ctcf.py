...
ALLOWED_DIRS = {
    Path("models"),
    # Path("experiments") / "OASIS",
    # Path("utils")
}

# Default output filename
DEFAULT_OUT = "diff_models.txt"
...
def iter_allowed_files(project_root: Path) -> List[Path]:
    """
    Enumerate all allowed files under ALLOWED_DIRS plus ALLOWED_FILES.
    Returned paths are absolute, sorted by relative path.
    """
    out: List[Path] = []

    # Add all files under allowed dirs
    for d in sorted(ALLOWED_DIRS):
        abs_d = (project_root / d).resolve()
        if abs_d.is_dir():
            for p in abs_d.rglob("*"):
                if p.is_file():
                    out.append(p)
        else:
            # Directory missing is not fatal; we just won't include it
            pass
    ...
    for p in out:
        try:
            ...
            continue
        uniq[str(rel).replace("\\", "/")] = p.resolve()

...
def validate(
    ...
    register_model_cls,
    mk_grid_img_fn: Callable[..., torch.Tensor] | None = None,
    grid_step: int = 8,
    line_thickness: int = 1,
    max_batches: int | None = None,
    ds: str = "OASIS",
) -> ValResult:
    """Validation loop returning protocol metrics plus last-batch visualisation tensors."""
    model.eval()
    reg_nearest = None
    reg_bilin = None
    ...
    def calc_jac(flow: torch.Tensor, x_seg: torch.Tensor):
        if is_ixi:
    ...
    for bidx, batch in enumerate(val_loader):
        ...
        jac_meter.update(jac, x.size(0))
        if sdlogj_meter is not None and sdlogj is not None:
            sdlogj_meter.update(sdlogj, x.size(0))
        if ndv_meter is not None and ndv is not None:
            ndv_meter.update(ndv, x.size(0))
    ...
    return ValResult(
        dsc=dsc_meter.avg,
        jac_nonpos_percent=jac_meter.avg,
        ndv_percent=ndv_avg,
        sdlogj=sdlogj_avg,
        last_vis=last_vis,
    )

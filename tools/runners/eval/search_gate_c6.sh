...
OUT_ROOT="${OUT_ROOT:-results/search_gate_c6}"
HEAVY_OUT_ROOT="${HEAVY_OUT_ROOT:-results/search_gate_c6_heavy}"
SOURCE_C4_DIR="${SOURCE_C4_DIR:-results/search_gate_c4/C4_DEVELOPMENT_20260824T161239Z_c69d12000176}"
...
if [[ -n "$GIT_STATUS_AT_START" ]]; then
  echo "[FAIL] Refusing to run C6 from a dirty tree:" >&2
  printf '%s\n' "$GIT_STATUS_AT_START" >&2
  echo "[HINT] Redirect nohup output to /tmp/search_gate_c6.log." >&2
  exit 1
fi
if [[ ! -f "$SOURCE_C4_DIR/c4_manifest.json" || ! -f "$SOURCE_C4_DIR/run_manifest.json" ]]; then
  echo "[FAIL] Frozen successful compact C4 source is absent: $SOURCE_C4_DIR" >&2
  exit 1
fi
if [[ ! -d "$SOURCE_C4_HEAVY_ROOT" ]]; then
  echo "[FAIL] Frozen C4 heavy source is absent: $SOURCE_C4_HEAVY_ROOT" >&2
  exit 1
fi
...
RUN_ID="${RUN_ID:-C6_DEVELOPMENT_$(sg_utc_run_stamp)_$(sg_git_short_head)}"
...
SOURCE_C4_DIR_ABS="$(realpath -m "$SOURCE_C4_DIR")"
...
if [[ "$HEAVY_RUN_ROOT_ABS" == "$RUN_ROOT_ABS" || "$HEAVY_RUN_ROOT_ABS" == "$RUN_ROOT_ABS/"* || \
  ...
  echo "[FAIL] Compact and heavy C6 roots must not overlap" >&2
  exit 2
fi
...
if ! flock -n 9; then
  echo "[FAIL] Another C6 process holds RUN_ID=$RUN_ID" >&2
  exit 1
fi
...
if [[ ! -f "$RUN_ROOT/runner_contract.txt" ]]; then
  ...
  echo "[FAIL] Resume settings differ from the original C6 RUN_ID contract" >&2
  exit 1
fi
...
if [[ -f "$STARTED_FILE" ]]; then STARTED_AT="$(<"$STARTED_FILE")"; else printf '%s\n' "$STARTED_AT" > "$STARTED_FILE"; fi
{
  ...
  printf 'SOURCE_C4_DIR=%q SOURCE_C4_HEAVY_ROOT=%q MIN_FREE_GIB=%q RUN_ID=%q ' \
    "$SOURCE_C4_DIR" "$SOURCE_C4_HEAVY_ROOT" "$MIN_FREE_GIB" "$RUN_ID"
...
package_and_finalize() {
  ...
  if [[ ! -f "$RUN_ROOT/heavy_retention.txt" ]]; then
    {
      ...
      printf 'target_c6_heavy=%s\n' "$HEAVY_RUN_ROOT_ABS"
      printf 'retention=RETAIN_ALL_THREE_ROOTS_UNTIL_EXPLICIT_OPERATOR_DECISION\npackaged=false\n'
  ...
  if [[ -e "$package" || -e "${package}.sha256" ]]; then
    echo "[FAIL] Refusing to overwrite an existing C6 package: $package" >&2
    exit 1
  fi
...
echo "########## C6 fail-closed self-check ##########"
"$PYBIN" -m tools.dev.check_transactional_search --output "$ATTEMPT_ROOT/transactional_selfcheck.json"
"$PYBIN" -m tools.analysis.run_search_gate_c6 selfcheck --output "$ATTEMPT_ROOT/c6_selfcheck.json"
for artifact in transactional_selfcheck.json c6_selfcheck.json; do
...
echo "########## C6 C4 source authentication and contracts ##########"
"$PYBIN" -m tools.analysis.run_search_gate_c6 prepare \
  --run-root "$RUN_ROOT" --heavy-root "$HEAVY_RUN_ROOT" \
  --source-c4-dir "$SOURCE_C4_DIR" --source-c4-heavy-root "$SOURCE_C4_HEAVY_ROOT" \
...
echo "########## C6 single-case real-H100 decision pilot ##########"
PILOT_LOG="$RUN_ROOT/workers/decision/attempts/$ATTEMPT_ID/pilot.log"
mkdir -p "$(dirname "$PILOT_LOG")"
CUDA_VISIBLE_DEVICES="${GPUS[0]}" "$PYBIN" -m tools.analysis.run_search_gate_c6 decision-pilot \
...
echo "########## C6 label-free decision workers ##########"
...
for shard_index in "${!GPUS[@]}"; do
  ...
  (
    CUDA_VISIBLE_DEVICES="$physical_gpu" "$PYBIN" -m tools.analysis.run_search_gate_c6 decision-worker \
      --run-root "$RUN_ROOT" --decision-contract-sha256 "$DECISION_SHA" \
...
for index in "${!PIDS[@]}"; do
  if ! wait "${PIDS[$index]}"; then echo "[FAIL] C6 decision shard $index failed; inspect its worker log." >&2; worker_failed=1; fi
done
PIDS=()
if [[ "$worker_failed" -ne 0 ]]; then exit 1; fi

echo "########## C6 immutable label barrier ##########"
"$PYBIN" -m tools.analysis.run_search_gate_c6 decision-barrier \
...
echo "########## C6 post-barrier evaluation contract ##########"
"$PYBIN" -m tools.analysis.run_search_gate_c6 freeze-evaluation \
...
echo "########## C6 post-barrier evaluation workers ##########"
...
for shard_index in "${!GPUS[@]}"; do
  ...
  (
    CUDA_VISIBLE_DEVICES="$physical_gpu" "$PYBIN" -m tools.analysis.run_search_gate_c6 evaluation-worker \
      --run-root "$RUN_ROOT" --source-contract-sha256 "$SOURCE_SHA" --decision-contract-sha256 "$DECISION_SHA" \
...
worker_failed=0
for index in "${!PIDS[@]}"; do
  if ! wait "${PIDS[$index]}"; then echo "[FAIL] C6 evaluation shard $index failed; inspect its worker log." >&2; worker_failed=1; fi
done
PIDS=()
if [[ "$worker_failed" -ne 0 ]]; then exit 1; fi

echo "########## C6 finalization ##########"
"$PYBIN" -m tools.analysis.run_search_gate_c6 finalize \
  --run-root "$RUN_ROOT" --source-contract-sha256 "$SOURCE_SHA" --decision-contract-sha256 "$DECISION_SHA" \
  --barrier-sha256 "$BARRIER_SHA" --evaluation-contract-sha256 "$EVALUATION_SHA"
STATUS="COMPLETE"
EXIT_CODE=0

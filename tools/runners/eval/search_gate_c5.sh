#!/usr/bin/env bash
# Gate C5: preregistered sparse-intensity 4x3x3 factorial on IXI validation-58.
set -euo pipefail
...
SEED=0
OUT_ROOT="${OUT_ROOT:-results/search_gate_c5}"
HEAVY_OUT_ROOT="${HEAVY_OUT_ROOT:-results/search_gate_c5_heavy}"
...
if [[ -n "$GIT_STATUS_AT_START" ]]; then
  ...
  echo "[HINT] Redirect nohup output outside the repository, for example to /tmp/search_gate_c5.log." >&2
  exit 1
fi
if [[ ! -d "$SOURCE_C4_DIR" || ! -f "$SOURCE_C4_DIR/c4_manifest.json" || \
      ! -f "$SOURCE_C4_DIR/run_manifest.json" ]]; then
  echo "[FAIL] Frozen compact C4 source is missing or incomplete: $SOURCE_C4_DIR" >&2
  exit 1
fi
if [[ ! -d "$SOURCE_C4_HEAVY_ROOT" ]]; then
  echo "[FAIL] Frozen heavy C4 source is missing: $SOURCE_C4_HEAVY_ROOT" >&2
  exit 1
fi
...
RUN_ID="${RUN_ID:-C5_DEVELOPMENT_$(sg_utc_run_stamp)_$(sg_git_short_head)}"
...
package_and_finalize() {
  ...
  if [[ ! -f "$RUN_ROOT/heavy_retention.txt" ]]; then
    {
      printf 'source_c3_heavy_root=UNKNOWN_BEFORE_SOURCE_AUTHENTICATION\n'
      printf 'source_c4_heavy_root=%s\n' "$SOURCE_C4_HEAVY_ROOT_ABS"
      printf 'target_c5_heavy_root=%s\n' "$HEAVY_RUN_ROOT_ABS"
      printf 'retention=RETAIN_ALL_SOURCE_AND_TARGET_HEAVY_ROOTS_UNTIL_EXPLICIT_OPERATOR_DECISION\n'
  ...
  echo "[NOTICE] No heavy root was packaged or deleted. Retain all three until an explicit operator decision."
...
echo "########## C5 fail-closed self-check ##########"
"$PYBIN" -m tools.dev.check_transactional_search --output "$ATTEMPT_ROOT/transactional_selfcheck.json"
"$PYBIN" -m tools.analysis.run_search_gate_c5 selfcheck --output "$ATTEMPT_ROOT/c5_selfcheck.json"
for artifact in transactional_selfcheck.json c5_selfcheck.json; do
  if [[ ! -f "$RUN_ROOT/$artifact" ]]; then
    cp "$ATTEMPT_ROOT/$artifact" "$RUN_ROOT/$artifact"
...
for shard_index in "${!GPUS[@]}"; do
  ...
  (
    CUDA_VISIBLE_DEVICES="$physical_gpu" "$PYBIN" -m tools.analysis.run_search_gate_c5 evaluation-worker \
      --run-root "$RUN_ROOT" \
      --source-contract-sha256 "$SOURCE_CONTRACT_SHA256" \
      --decision-contract-sha256 "$DECISION_CONTRACT_SHA256" \
      --barrier-sha256 "$BARRIER_SHA256" \
      --evaluation-contract-sha256 "$EVALUATION_CONTRACT_SHA256" \
      --shard-index "$shard_index" --num-shards "${#GPUS[@]}" --gpu 0 \
      --physical-gpu "$physical_gpu" --attempt-id "$ATTEMPT_ID"
  ) > "$log" 2>&1 &
  PIDS+=("$!")
...
for index in "${!PIDS[@]}"; do
  if ! wait "${PIDS[$index]}"; then
    echo "[FAIL] C5 evaluation shard $index failed; inspect its worker log." >&2
    worker_failed=1
  fi
done
PIDS=()
...
echo "########## C5 compact finalization ##########"
"$PYBIN" -m tools.analysis.run_search_gate_c5 finalize \
  --run-root "$RUN_ROOT" \
  --source-contract-sha256 "$SOURCE_CONTRACT_SHA256" \
  --decision-contract-sha256 "$DECISION_CONTRACT_SHA256" \
  --barrier-sha256 "$BARRIER_SHA256" \

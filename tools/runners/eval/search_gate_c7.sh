#!/usr/bin/env bash
# Gate C7: frozen CorrMLP-x1 descriptor versus matched intensity on IXI validation-58.
set -euo pipefail
...
OUT_ROOT="${OUT_ROOT:-results/search_gate_c7}"
HEAVY_OUT_ROOT="${HEAVY_OUT_ROOT:-results/search_gate_c7_heavy}"
SOURCE_C6_DIR="${SOURCE_C6_DIR:-results/search_gate_c6/C6_DEVELOPMENT_20260827T211512Z_c0a59d1c04af}"
...
SOURCE_C6_HEAVY_ROOT="${SOURCE_C6_HEAVY_ROOT:-results/search_gate_c6_heavy/C6_DEVELOPMENT_20260827T211512Z_c0a59d1c04af}"
DESCRIPTOR_CHECKPOINT="${DESCRIPTOR_CHECKPOINT:-results/P11_CORRMLP_IXI/ckpt/last.pth}"
...
if [[ -n "$GIT_STATUS_AT_START" ]]; then
  echo "[FAIL] Refusing to run C7 from a dirty tree:" >&2
  printf '%s\n' "$GIT_STATUS_AT_START" >&2
  echo "[HINT] Redirect nohup output to /tmp/search_gate_c7.log." >&2
  exit 1
fi
for required in \
  "$SOURCE_C6_DIR/c6_manifest.json" \
  "$SOURCE_C6_DIR/decision_contract.json" \
  "$SOURCE_C6_DIR/decision_barrier.json" \
  "$DESCRIPTOR_CHECKPOINT"; do
  if [[ ! -f "$required" ]]; then
    echo "[FAIL] Frozen C7 input is absent: $required" >&2
    exit 1
  fi
done
for required in "$SOURCE_C3_HEAVY_ROOT" "$SOURCE_C4_HEAVY_ROOT" "$SOURCE_C6_HEAVY_ROOT"; do
  if [[ ! -d "$required" ]]; then
    echo "[FAIL] Frozen C7 heavy source is absent: $required" >&2
    exit 1
  fi
done
...
RUN_ID="${RUN_ID:-C7_DEVELOPMENT_$(sg_utc_run_stamp)_$(sg_git_short_head)}"
...
SOURCE_C6_DIR_ABS="$(realpath -m "$SOURCE_C6_DIR")"
SOURCE_C3_HEAVY_ROOT_ABS="$(realpath -m "$SOURCE_C3_HEAVY_ROOT")"
SOURCE_C4_HEAVY_ROOT_ABS="$(realpath -m "$SOURCE_C4_HEAVY_ROOT")"
SOURCE_C6_HEAVY_ROOT_ABS="$(realpath -m "$SOURCE_C6_HEAVY_ROOT")"
DESCRIPTOR_CHECKPOINT_ABS="$(realpath -m "$DESCRIPTOR_CHECKPOINT")"
if [[ "$HEAVY_RUN_ROOT_ABS" == "$RUN_ROOT_ABS" || "$HEAVY_RUN_ROOT_ABS" == "$RUN_ROOT_ABS/"* || \
      "$RUN_ROOT_ABS" == "$HEAVY_RUN_ROOT_ABS/"* ]]; then
  echo "[FAIL] Compact and heavy C7 roots must not overlap" >&2
  exit 2
fi
...
if ! flock -n 9; then
  echo "[FAIL] Another C7 process holds RUN_ID=$RUN_ID" >&2
  exit 1
fi
...
{
  printf 'gpu_list=%s\n' "$GPU_CANONICAL"
  printf 'protocol_id=%s\n' 'CTCF-SEARCH-GATE-C7-V1'
  printf 'git_head=%s\n' "$HEAD"
  printf 'paths_profile=%s\n' "$PATHS_PROFILE"
  printf 'compact_run_root=%s\n' "$RUN_ROOT_ABS"
  printf 'heavy_run_root=%s\n' "$HEAVY_RUN_ROOT_ABS"
  printf 'source_c6_dir=%s\n' "$SOURCE_C6_DIR_ABS"
  printf 'source_c3_heavy_root=%s\n' "$SOURCE_C3_HEAVY_ROOT_ABS"
  printf 'source_c4_heavy_root=%s\n' "$SOURCE_C4_HEAVY_ROOT_ABS"
  printf 'source_c6_heavy_root=%s\n' "$SOURCE_C6_HEAVY_ROOT_ABS"
  printf 'descriptor_checkpoint=%s\n' "$DESCRIPTOR_CHECKPOINT_ABS"
...
if [[ ! -f "$RUN_ROOT/runner_contract.txt" ]]; then
  ...
  echo "[FAIL] Resume settings differ from the original C7 RUN_ID contract" >&2
  exit 1
fi
...
if [[ -f "$STARTED_FILE" ]]; then STARTED_AT="$(<"$STARTED_FILE")"; else printf '%s\n' "$STARTED_AT" > "$STARTED_FILE"; fi
{
  ...
  printf 'SOURCE_C6_DIR=%q SOURCE_C3_HEAVY_ROOT=%q SOURCE_C4_HEAVY_ROOT=%q SOURCE_C6_HEAVY_ROOT=%q ' \
    "$SOURCE_C6_DIR" "$SOURCE_C3_HEAVY_ROOT" "$SOURCE_C4_HEAVY_ROOT" "$SOURCE_C6_HEAVY_ROOT"
  printf 'DESCRIPTOR_CHECKPOINT=%q MIN_FREE_GIB=%q RUN_ID=%q ATTEMPT_ID=%q REMOTE_LOCATOR=%q bash %q\n' \
    "$DESCRIPTOR_CHECKPOINT" "$MIN_FREE_GIB" "$RUN_ID" "$ATTEMPT_ID" "$REMOTE_LOCATOR" "$0"
...
package_and_finalize() {
  ...
  if [[ ! -f "$RUN_ROOT/heavy_retention.txt" ]]; then
    {
      printf 'source_c3_heavy=%s\n' "$SOURCE_C3_HEAVY_ROOT_ABS"
      printf 'source_c4_heavy=%s\n' "$SOURCE_C4_HEAVY_ROOT_ABS"
      printf 'source_c6_heavy=%s\n' "$SOURCE_C6_HEAVY_ROOT_ABS"
      printf 'target_c7_heavy=%s\n' "$HEAVY_RUN_ROOT_ABS"
      printf 'retention=RETAIN_SOURCE_AND_TARGET_HEAVY_ROOTS_UNTIL_EXPLICIT_OPERATOR_DECISION\npackaged=false\n'
  ...
  if [[ "$STATUS" == "COMPLETE" ]]; then package_base="$RUN_ID"; else package_base="${RUN_ID}__${ATTEMPT_ID}__FAILED"; fi
  package="results/exports/${package_base}.tar.gz"
  package_abs="$(pwd)/$package"
  if [[ -z "$REMOTE_LOCATOR" ]]; then REMOTE_LOCATOR="$(sg_default_remote_locator "$package_abs")"; fi
  {
    printf 'archive=%s\n' "$package_abs"
  ...
  "$PYBIN" -m tools.analysis.run_artifacts finalize \
    ...
    --time-steps 3 --expected-preflights 1 --strict-checkpoint-load --remote-locator "$REMOTE_LOCATOR" \
    || { STATUS="FAILED"; EXIT_CODE=1; }
  if [[ -e "$package" || -e "${package}.sha256" ]]; then
    echo "[FAIL] Refusing to overwrite an existing C7 package: $package" >&2
    exit 1
  fi
...
echo "########## C7 fail-closed self-check ##########"
"$PYBIN" -m tools.dev.check_transactional_search --output "$ATTEMPT_ROOT/transactional_selfcheck.json"
"$PYBIN" -m tools.analysis.run_search_gate_c7 selfcheck --output "$ATTEMPT_ROOT/c7_selfcheck.json"
for artifact in transactional_selfcheck.json c7_selfcheck.json; do
  if [[ ! -f "$RUN_ROOT/$artifact" ]]; then cp "$ATTEMPT_ROOT/$artifact" "$RUN_ROOT/$artifact"
...
echo "########## C7 C6 source and descriptor authentication ##########"
"$PYBIN" -m tools.analysis.run_search_gate_c7 prepare \
  --run-root "$RUN_ROOT" --heavy-root "$HEAVY_RUN_ROOT" \
  --source-c6-dir "$SOURCE_C6_DIR" \
  --source-c3-heavy-root "$SOURCE_C3_HEAVY_ROOT" \
  --source-c4-heavy-root "$SOURCE_C4_HEAVY_ROOT" \
  --source-c6-heavy-root "$SOURCE_C6_HEAVY_ROOT" \
  --descriptor-checkpoint "$DESCRIPTOR_CHECKPOINT" \
...
echo "########## C7 single-case descriptor and decision pilot ##########"
PILOT_LOG="$RUN_ROOT/workers/decision/attempts/$ATTEMPT_ID/pilot.log"
mkdir -p "$(dirname "$PILOT_LOG")"
CUDA_VISIBLE_DEVICES="${GPUS[0]}" "$PYBIN" -m tools.analysis.run_search_gate_c7 decision-pilot \
...
echo "########## C7 label-free decision workers ##########"
...
for shard_index in "${!GPUS[@]}"; do
  ...
  (
    CUDA_VISIBLE_DEVICES="$physical_gpu" "$PYBIN" -m tools.analysis.run_search_gate_c7 decision-worker \
      --run-root "$RUN_ROOT" --decision-contract-sha256 "$DECISION_SHA" \
...
for index in "${!PIDS[@]}"; do
  if ! wait "${PIDS[$index]}"; then echo "[FAIL] C7 decision shard $index failed; inspect its worker log." >&2; worker_failed=1; fi
done
PIDS=()
if [[ "$worker_failed" -ne 0 ]]; then exit 1; fi

echo "########## C7 immutable label barrier ##########"
"$PYBIN" -m tools.analysis.run_search_gate_c7 decision-barrier \
  --run-root "$RUN_ROOT" --decision-contract-sha256 "$DECISION_SHA" --attempt-id "$ATTEMPT_ID"
BARRIER_SHA="$(sha256sum "$RUN_ROOT/decision_barrier.json" | awk '{print $1}')"

echo "########## C7 post-barrier evaluation contract ##########"
"$PYBIN" -m tools.analysis.run_search_gate_c7 freeze-evaluation \
...
echo "########## C7 post-barrier evaluation workers ##########"
...
for shard_index in "${!GPUS[@]}"; do
  ...
  (
    CUDA_VISIBLE_DEVICES="$physical_gpu" "$PYBIN" -m tools.analysis.run_search_gate_c7 evaluation-worker \
      --run-root "$RUN_ROOT" --source-contract-sha256 "$SOURCE_SHA" --decision-contract-sha256 "$DECISION_SHA" \
...
worker_failed=0
for index in "${!PIDS[@]}"; do
  if ! wait "${PIDS[$index]}"; then echo "[FAIL] C7 evaluation shard $index failed; inspect its worker log." >&2; worker_failed=1; fi
done
PIDS=()
if [[ "$worker_failed" -ne 0 ]]; then exit 1; fi

echo "########## C7 finalization ##########"
"$PYBIN" -m tools.analysis.run_search_gate_c7 finalize \
  --run-root "$RUN_ROOT" --source-contract-sha256 "$SOURCE_SHA" --decision-contract-sha256 "$DECISION_SHA" \
  --barrier-sha256 "$BARRIER_SHA" --evaluation-contract-sha256 "$EVALUATION_SHA"
STATUS="COMPLETE"
EXIT_CODE=0

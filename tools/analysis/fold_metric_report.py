...
from tools.analysis.compute_stats import benjamini_hochberg, hodges_lehmann_paired  # noqa: E402
...
def main():
    ...
    for d in sorted(Path(a.inference_dir).iterdir()):
        ...
        if df is None:
            continue
        rows.append(
            dict(exp=d.name, n=len(df), dice=df.dice_mean.mean(), central=df[CENTRAL].mean(),
                 corners=df[CORNERS].mean(), digital=df[DIGITAL].mean(), ndv=df[NDV].mean())
        )
    t = pd.DataFrame(rows)
    if t.empty:
    ...
    md = ["# Fold count vs finite-difference scheme\n",
    ...
    for _, r in t.iterrows():
        ...
        md.append(f"| {r.exp} | {r.n} | {r.dice:.4f} | {r.central:.4f} | {r.corners:.4f} | "
    ...
    for lo, hi, lbl in ((0, 0.01, "< 0.01 (looks perfect)"), (0.01, 0.5, "0.01 – 0.5"),
    ...
    if base is not None:
        md += [f"## Paired tests vs `{Path(a.baseline).name}`\n",
        ...
        for metric, label in ((CENTRAL, "central"), (DIGITAL, "digital-10")):
            ...
            for _, r in t.iterrows():
                ...
                m = df[["case_id", metric]].merge(
                    base[["case_id", metric]], on="case_id", suffixes=("_a", "_b")
                )
                if len(m) != len(base):
                    continue
            ...
            for lb, x, qq in zip(labels, res, q):
                md.append(f"| {lb} | {label} | {x['hl']:+.4f} | "

...
from TransMorph.models.utils_field import warp, sdlogj_metric
from TransMorph.models.losses_ctcf import LNCCLoss, Grad3dLoss
...
def main():
    ...
    ap.add_argument('--exp',       default='TM_Baseline')
    ...
    # ---------- model (одноступенчатый TransMorph) ----------
    base_cfg = CONFIGS_TM[C.base_backbone]
    base_cfg.img_size = tuple(C.img_size)
    model = BaseRegModel(base_cfg).to(device)
    ...
    for epoch in range(1, hp.epochs + 1):
        ...
        for i, data in enumerate(train_loader):
            ...
            with autocast_ctx:
                ...
                L_reg = reg_grad(phi_ab)
                loss = L_sim + 0.5 * L_reg

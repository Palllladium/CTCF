...
def get_UTSRMorphIXILarge_config():
    """IXI-specific Large config from original repo (embed=96, deeper Swin)."""
    config = ml_collections.ConfigDict()
    config.if_transskip = True
    config.if_convskip = True
    config.patch_size = 4
...
CONFIGS = {
    "UTSRMorph": get_UTSRMorph_config(),
    "UTSRMorph-Large": get_UTSRMorphLarge_config(),
    "UTSRMorph-IXI-Large": get_UTSRMorphIXILarge_config(),
    "UTSRMorph-Debug": get_UTSRMorph_debug_config(),
}

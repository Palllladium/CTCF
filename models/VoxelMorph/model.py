...
class unet_core(nn.Module):
    ...
    def __init__(self, dim, enc_nf, dec_nf, full_size=True):
        ...
        if self.full_size: self.dec.append(conv_block(dim, dec_nf[4] + 2, dec_nf[5], 1))
        if self.vm2: self.vm2_conv = conv_block(dim, dec_nf[5], dec_nf[6]) 
...
class conv_block(nn.Module):
    ...
    def __init__(self, dim, in_channels, out_channels, stride=1):
        ...
        if stride == 1: ksize = 3
        elif stride == 2: ksize = 4
        else: raise Exception('stride must be 1 or 2')

"""
VMambaMorph: Visual Mamba-based Framework for 3D Image Registration
(arxiv 2404.05105). Self-contained port of the relevant classes from the
upstream repo (mambamorph/torch/{vmamba.py,mamba.py,TransMorph.py}).

Key architectural difference from MambaMorph:
  - VMambaMorph replaces the single-scan `MambaLayer` with a stack of
    `VSSBlock`s using `SS2D`, which performs a 4-direction cross-scan
    (forward + reverse along H and W axes) for non-causal 3D data.

Self-contained structure:
  - 4-direction `SS2D` selective-scan block — defined here.
  - `VSSBlock` and `VMambaLayer` — defined here.
  - `VMambaBlock` (encoder) — defined here.
  - `VMambaMorph` model — defined here.
...
class SS2D(nn.Module):
    ...
    def __init__(self, d_model, d_state=16, d_conv=3, expand=2,
                 ...
                 dt_scale=1.0, dt_init_floor=1e-4, dropout=0.0,
        ...
        self.conv3d = nn.Conv3d(
            ...
            kernel_size=d_conv, padding=(d_conv - 1) // 2, **factory,
        )
        self.act = nn.SiLU()

        # 4-direction projections: stacked weights (K=4)
        x_proj = tuple(
            nn.Linear(self.d_inner, self.dt_rank + self.d_state * 2, bias=False, **factory)
            for _ in range(4)
        )
        ...
        dt_projs = tuple(
            self._dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init,
                          dt_min, dt_max, dt_init_floor, **factory)
            for _ in range(4)
        )
    ...
    def _dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random",
        ...
        dt = torch.exp(
            torch.rand(d_inner, **factory) * (math.log(dt_max) - math.log(dt_min)) + math.log(dt_min)
...
class VMambaLayer(nn.Module):
    def __init__(self, dim, depths, d_state=16, d_conv=4, expand=2, downsample=None):
        ...
        self.mamba = nn.ModuleList([
            VSSBlock(hidden_dim=dim, drop_path=0.0, norm_layer=nn.LayerNorm,
                     attn_drop_rate=0.0, d_state=d_state)
            for _ in range(depths)
        ])
        self.downsample = (
            downsample(dim=dim, norm_layer=nn.LayerNorm, reduce_factor=4)
...
class VMambaBlock(nn.Module):
    def __init__(self, patch_size=4, in_chans=2, embed_dim=96,
                 depths=(2, 2, 2), drop_rate=0.0, norm_layer=nn.LayerNorm,
                 ape=False, spe=False, rpe=True, patch_norm=True,
                 out_indices=(0, 1, 2), frozen_stages=-1,
                 d_state=16, d_conv=4, expand=2, pretrain_img_size=224):
        super().__init__()
...
class VMambaMorph(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = VMambaBlock(
            ...
            embed_dim=embed_dim, depths=config.depths,
            drop_rate=config.drop_rate, ape=config.ape, spe=config.spe,
            rpe=config.rpe, patch_norm=config.patch_norm,

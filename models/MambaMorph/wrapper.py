...
class MambaMorphSolo(nn.Module):
    ...
    def __init__(self, config_key: str = "MambaMorph", diffeomorphic: bool = True, img_size=None):
        ...
        if config_key not in _CFG_REGISTRY:
            raise KeyError(f"Unknown MambaMorph config '{config_key}'. Available: {list(_CFG_REGISTRY.keys())}")
    ...
    def preint_flow_fn(self):
        ...
        def _preint(mov, fix):
            return self.model(mov, fix)["preint_flow"]
...
class MambaMorphCascadeL2(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        config_key = getattr(config, "mamba_config", "MambaMorph")
        if config_key not in _CFG_REGISTRY:
            raise KeyError(f"Unknown MambaMorph config '{config_key}'. Available: {list(_CFG_REGISTRY.keys())}")
